 ...
 void SPIRVOpenCLBuiltinsEmitter::emitSelectRecords(raw_ostream &OS){
   ...
   OS << "static bool generateSPIRVFromOpenCL(StringRef mangledName) {\n\n  ";
   for(auto const &record: OCLRecords){
     StringRef oclFunctionName = record->getValueAsString("opencl_mangled");
     unsigned resultOpCode = record->getValueAsInt("opcode_target");
     OS << "if(mangledName.startswith( "  << oclFunctionName << ") return true;\n  ";
